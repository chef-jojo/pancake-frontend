...
import type { BlockResponse } from '@/components/SubgraphHealthIndicator'
...
const fetchAndUpdateLPsAPR = async () => {
  ...
  fs.writeFile(`packages/core/config/constants/lpAprs.json`, JSON.stringify(allAprs, null, 2) + os.EOL, (err) => {
    if (err) throw err
