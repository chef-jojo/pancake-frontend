...
export const getBorderBackground = (theme: DefaultTheme, status: Status) => {
  ...
  switch (status) {
    case 'calculating':
      return `linear-gradient(transparent ${gradientStopPoint}, ${theme.colors.cardBorder} ${gradientStopPoint}), ${theme.colors.gradientsCardHeader}`
    case 'canceled':
...
const getBackgroundColor = (theme: DefaultTheme, status: Status) => {
  switch (status) {
    case 'calculating':
      return theme.colors.gradientsCardHeader
    case 'live':
      return 'transparent'
    case 'canceled':
